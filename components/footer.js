    ...
    return (
        <footer className={'footer '+ (navClass ? 'fixed md:relative' : 'relative')}>
            ...
            <div className={'relative w-full h-full px-9 md:px-12 lg:px-20 xl:px-24 3xl:px-46 pt-16 lg:pt-24 xl:pt-32 2xl:pt-48 hidden md:block'}>
                <div className='footer__container 3xl:container mx-auto grid lg:grid-cols-4 pt-12 lg:pt-16 pb-2 md:pb-8 lg:pb-16'>
                    <div className='lg:col-span-3'>
                        <div className='grid grid-cols-3 xl:flex xl:flex-nowrap gap-4 xl:gap-24'>
                            {navigation.map( (nav, kNav) => {
                                ...
                                return (
                                    <div key={"nav-"+kNav}>
                                        <span className='text-white font-medium font-lato text-sm xl:text-lg 2xl:text-22 uppercase tracking-xl'>{TitleSection}</span>
                                            ...
                                            {Links.map( (l, key) => {
                                                return (
                                                    ...
                                                    <PrismicLink field={l.Link} key={"prismicLink-" + key}>
                                                        <span className='text-white text-sm xl:text-lg 2xl:text-2xl text-medium' key={"link-" + key} style={{ color : l.textColor ? l.textColor : "white" }}>{l.LinkText}</span>
                                                    </PrismicLink>
                                                    :
                                                    <span className='text-white text-sm xl:text-lg 2xl:text-2xl text-medium cursor-pointer' key={"_" + key} style={{ color : l.textColor ? l.textColor : "white" }}>{l.LinkText}</span>
                                                )
                                            } )}
                                        </div>
                                    </div>
                        ...
                        <div className='footer__text-made hidden lg:block pt-24 xl:pt-32 2xl:pt-64 3xl:pt-80 text-white tracking-xl font-bold uppercase text-sm xl:text-lg 2xl:text-xl'><PrismicRichText field={FooterText}/></div>
                    </div>
                    <div className='lg:col-span-1'>
                        <div className='flex flex-col justify-between h-full text-jw-green-1'>
                            ...
                            <div className='flex justify-between items-end md:pt-16 lg:pt-32 2xl:pt-48'>
                                <div className='footer__text-made block lg:hidden text-white tracking-xl font-bold uppercase text-sm xl:text-lg 2xl:text-xl'><PrismicRichText field={FooterText}/></div>
                                <div className='font-lato text-sm xl:text-lg 2xl:text-22 tracking-xl pb-10'>
                                    {NMLS}
                                </div>
                            </div>
            ...
            <div className={'block md:hidden bg-footer-bottom '+ (navClass ? 'pt-12' : '')}>
                <div className={'footer__store flex justify-center gap-4 text-white px-12 ' + (navClass ? 'block' : 'hidden')}>
                    <span className=''><Icon icon={'apple'} /></span>
                    <span className=''><Icon icon={'android'} /></span>
                </div>
                <div className={'footer__policies flex justify-between items-end px-8 pb-8 text-xxs xs:text-9 '+ (navClass ? 'pt-6 xs:pt-12' : 'py-8')}>
